  ...
  @@ group "omnomnom"
       [ (let work ~name old_contents =
            ...
            let new_contents =
              ...
              |> Printf.sprintf "data/diff/%s.new"
              |> open_in |> Omnomnom__.Io_util.read_all
            in
            diff
          ...
          in
          OmnomnomGolden.of_directory work ~rename:(fun x -> x ^ ".diff") ~group:"Generates diffs" ~directory:"data/diff"
            ~extension:".old" ())
       ]
